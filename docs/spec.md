...
- 地点
  - 現在地取得（Geolocation API + BigDataCloud reverse geocode）
  - 地点検索（Open-Meteo Geocoding + ローカルフォールバック辞書、市町村サフィックス自動展開）
  - 位置情報失敗時は東京 fallback、再取得時は静かな通知のみ
- 気象データ表示
  - 気圧 / 気温 / 天気 / 降水 / 風 / 湿度 / 花粉
  - 24H / 3D / 7D / 14D グラフ（3D 以上は hourly データで滑らかに描画、X 軸は日単位で間引き）
  - 「現在 (HH:MM)」ラベル付きの基準線、現在の表示位置を中央 / 左端で切替
- アドバイスカード
  - 服装 / 持ち物 / 行動の各カードを、Timeline と同期した時間帯ごとに表示
  - チェック状態は localStorage に保存。日付が変わった分は起動時に自動掃除
- 今日のサマリー
  - 現在時刻の hourly データから天気名を表示
  - 今日の最高 / 最低気温と、現在時刻スロットの気温・気圧トレンド・降水確率・風を 4 stat で表示
  - 状態別マスコット（晴れ / 雨 / 花粉 / 低気圧 / 風強い / 安定）+ クリックで一言ポップオーバー
- 今日の空だより
  - 気圧変動 / 強雨 / 高花粉 / 気温差 / 通常 / データ無し の 6 パターン
  - 季節（春・夏・秋・冬）別の追加メッセージプール
  - マスコットクリックで縦書きの一言（35 パターン、ランダム）
  - 文末に季節 / 気象別スタンプ
- 季節の言葉カード
  - 二十四節気 + 七十二候 を期間タグ付きで表示、各タームの description 併記
- 設定
  - 服装ジャンル / 体質補正（寒がり・暑がり）/ シーン（室内・室外）の保存
  - 「そらリスのこと」ポップオーバーで紹介文
- PWA
  - manifest / service worker (Serwist) / オフラインフォールバック画面 / インストール対応
  - app-icon (192/512/maskable) と OGP / Twitter Card メタを完備
- 遊び心 UI
  - マスコット 3 体に常時の控えめなバウンス + ホバー時の黄み発光
  - フッター背面の雲アニメーション
  - 時間帯（朝・昼・夕・夜）で背景グラデを微変化
  - アドバイスカードのチェック時にどんぐりが転がる
  - Easter egg: フッター © 横の小さなどんぐりボタンでアコーン軌道アニメ

### 11.2 簡易実装

花粉カードの分類精度（種別表示は API 制約により概算）/ AI 風アドバイスは固定パターン + 季節差分の組み合わせ。

### 11.3 準備中表示

花粉アラート / 月の満ち欠け / 紫外線詳細 / LINE 通知 / 週次レポート / アカウント同期 / 課金 / スキン / モバイルアプリ。
...
```
Open-Meteo Forecast / Air Quality / Geocoding (raw)
  ↓ normalizeWeatherData
NormalizedWeather / NormalizedPollen
  ↓ buildTimeSlots(range, anchor, dayOffset)
TimeSlot[]
  ↓ buildWeatherConditions(slots, weather, pollen)
WeatherCondition[]                    ← SummaryCard 等が消費
  ↓ buildRecommendations(conditions, profile)
{ outfit, carry, action, letter }     ← TimeSyncedSection / SkyLetterCard が消費
  ↓
UI 表示
```
...
可視化のためのシーケンス図・コンポーネント図は [`docs/architecture.md`](architecture.md) に集約。

### 12.2 永続化（localStorage）

zustand persist で `sora-risu:settings` キーに保存。

| 項目 | 内容 |
| --- | --- |
| `location` | 選択中の地点 (`GeoLocation`) |
| `profile` | 服装ジャンル / 体質 / シーン |
| `timelineRange` / `displayTarget` / `chartSeries` / `chartAnchor` | 表示状態 |
| `carryChecks` / `outfitChecks` / `actionChecks` | アドバイスカードのチェック状態 (slotId → bool) |

起動時 (`onRehydrateStorage`) に各 checks マップを走査し、**キーの先頭 `YYYY-MM-DD` が今日より前**のものを破棄。これでカレンダー由来の slot ID が日々増えてストレージを圧迫するのを防ぐ。

`version` フィールドを使った `migrate` で過去スキーマからのマイグレーションを行う。新しいフィールドを追加した時は `version` を 1 進めて migrate に分岐を足す。
...
src/
  app/
    layout.tsx           # メタデータ / OGP / Twitter Card / PWA manifest
    page.tsx             # ダッシュボード本体
    providers.tsx        # React Query Provider
    sw.ts / offline/     # Service Worker (Serwist) / オフライン画面
    globals.css          # キーフレーム (acorn-spin / mascot-bounce 等) / トークン
  components/
    layout/
      AppShell.tsx              # ページ枠、時間帯背景、フッター雲
      LocationHeader.tsx
      TimeBasedBackground.tsx   # 朝/昼/夕/夜 グラデ
      FooterCloudAnimation.tsx
    dashboard/
      SummaryCard.tsx           # 今日のサマリー + 状態別マスコット
      TimelinePanel.tsx
      ChartSeriesPicker.tsx / ChartAnchorToggle.tsx
      WeatherChart.tsx          # 気圧/気温/降水/天気/花粉のマルチチャート
      TimeSyncedSection.tsx     # 時間帯ごとのアドバイスカード集約
    cards/
      OutfitItemCard.tsx / CarryItemCard.tsx / ActionCard.tsx
      SkyLetterCard.tsx         # 今日の空だより
      SeasonalWordCard.tsx      # 二十四節気 + 七十二候
    settings/
      SettingsPanel.tsx
    brand/
      AcornIcon.tsx / AcornLoader.tsx / AcornRoll.tsx
      SoraRisuPopover.tsx       # 状態別マスコット + ポップオーバー
      SkyMascotChirp.tsx        # 縦書き一言ポップオーバー
      SeasonalStamp.tsx
      SecretAcornButton.tsx     # Easter egg
      MiniMascot.tsx / BrandMark.tsx
    weather/
      useWeather.ts             # React Query フック
    ui/
      (shadcn 風プリミティブ)
  features/
    weather/
      api/openMeteoClient.ts / airQualityClient.ts
      api/geocodingClient.ts / geocodingSearch.ts / reverseGeocodingClient.ts
      mappers/normalizeWeatherData.ts
      services/buildTimeSlots.ts / buildWeatherConditions.ts
    recommendations/
      clothingRules.ts / carryItemRules.ts / actionRules.ts
      letterMessages.ts / letterRules.ts
      risuMood.ts               # SummaryCard マスコットのポーズ判定
      buildRecommendations.ts
    seasonal/
      seasonalWords.ts          # 七十二候
      solarTerms.ts             # 二十四節気
  stores/
    useAppStore.ts              # zustand persist + 起動時自動掃除
  types/
    location.ts / weather.ts / recommendation.ts / timeline.ts / settings.ts
  lib/
    cn.ts / labels.ts / periodTone.ts
```
...
- 静か / やさしい / 上品 / 安心 / 朝に開きたくなる
- 子どもっぽくしすぎない、医療っぽくない、天気アプリっぽくない
- 情報の主役はあくまで気圧・天気・服装・持ち物・行動。マスコットは案内役

### 14.2 アニメーション基準

| 要素 | 振幅 / 速度 | 備考 |
| --- | --- | --- |
| AcornIcon (サマリー / 空だより / 設定の見出し) | translateY 0→-12% / 1.8s | カードのタイトル横でだけ有効 |
| マスコット 3 体 (SoraRisuPopover / SkyMascotChirp) | translateY 0→-2.5% / 2.4s | 常時バウンス、ホバー時は停止して発光に切替 |
| ホバー発光 | drop-shadow 16px 黄み + 6px 暖色 | 240ms ease-out で点滅 |
| どんぐりロール (チェック時) | left -10%→110% / 1.1s 1 回 | unchecked→checked の遷移時のみ |
| Easter egg アコーン軌道 | 1.8〜3.6s ランダム / 回転 ±720〜1800° | キーフレーム CSS 変数で毎回ランダム |
| フッター雲 | 120s linear 無限 (background-position) | mix-blend-mode: multiply |
| 時間帯背景 | 700ms transition | 5 分おきに hour 検査して位相切替 |

すべて `prefers-reduced-motion: reduce` で停止する `motion-*` クラスに紐付け。

### 14.3 状態判定の優先順位

`pickRisuMood(highlight)` は以下の優先度で評価し、最初にマッチした 1 ポーズを返す。

1. データ無し → `on-cloud` + 待ちメッセージ
2. `pressure.changeLevel === "high"` → `pressure_low`（心配顔）
3. `precipitation.level === "high"` → `umbrella`
4. `pollen.level === "high" | "very_high"` → `mask`
5. `wind.level === "high"` → `blanket`（風除け）
6. `precipitation.level === "medium"` → `rain`
7. `pressure.trend === "stable"` → `pressure_calm`（どんぐり抱え）
8. その他 → `sunny`（にこにこ）

### 14.4 季節スタンプ

`pickStampKind(category, date)` は SkyLetter の `category` と現在月から 6 種から選ぶ。

- `category === "rain"` → 雨スタンプ
- `category === "pollen"` → 葉スタンプ
- それ以外 → 月で春（花）/ 夏（日傘）/ 秋（どんぐり）/ 冬（手袋）

### 14.5 一言ポップオーバー（SkyMascotChirp）

10 文字以内のメッセージ 35 パターンから、クリックごとに前回と異なるものを抽選。縦書き (`writing-mode: vertical-rl` + `text-orientation: upright`) で 13px、行間 6px。
